...
async def test_capacity_zero_disables_limiter() -> None:
    ...
    for _ in range(100):
        allowed, retry = await rl.acquire(
            route="x", key="ip", capacity=0, refill_per_minute=0
        )
...
async def test_separate_keys_have_separate_buckets() -> None:
    ...
    for _ in range(2):
        allowed, _ = await rl.acquire(
            route="login", key="a", capacity=2, refill_per_minute=60
        )
        assert allowed is True
    # `a` is out, but `b` still has a full bucket
    allowed, _ = await rl.acquire(
        route="login", key="b", capacity=2, refill_per_minute=60
    )
    assert allowed is True
...
async def test_separate_routes_have_separate_buckets() -> None:
    ...
    for _ in range(2):
        allowed, _ = await rl.acquire(
            route="login", key="x", capacity=2, refill_per_minute=60
        )
        assert allowed is True
    allowed, _ = await rl.acquire(
        route="signup", key="x", capacity=2, refill_per_minute=60
    )
...
async def test_refill_eventually_unblocks() -> None:
    ...
    allowed, _ = await rl.acquire(
        route="r", key="x", capacity=1, refill_per_minute=60
    )
    assert allowed is True
    allowed, _ = await rl.acquire(
        route="r", key="x", capacity=1, refill_per_minute=60
