...
from livepeer_open_clearinghouse.domains.accounts.repo import User, UserSession
from livepeer_open_clearinghouse.domains.accounts.types import (
...
async def logout_endpoint(
    ...
    settings: SettingsDep,
    response: Response,
    open_clearinghouse_session: Annotated[
    ...
    """
    if open_clearinghouse_session:
        serializer = session_helper.make_serializer(settings.session_secret.get_secret_value())
        max_age = int(service.SESSION_TTL.total_seconds())
        raw = session_helper.unseal(serializer, open_clearinghouse_session, max_age_seconds=max_age)
        if raw:
...
async def oauth_providers_endpoint() -> dict[str, list[str]]:
    ...
    return {"enabled": [p for p in ("google", "github") if oauth_is_enabled(p)]}


def _redirect_uri(request: Request, provider: str) -> str:
    return str(request.url_for("oauth_callback_endpoint", provider=provider))
