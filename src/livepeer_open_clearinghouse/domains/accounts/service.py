...
from livepeer_open_clearinghouse.domains.accounts.repo import (
    OperatorApproval,
    User,
...
from livepeer_open_clearinghouse.providers.auth import password as pwd
...
async def verify_email(session: AsyncSession, *, token: str, clock: Clock) -> User:
...
async def resolve_session(session: AsyncSession, *, raw_token: str, clock: Clock) -> User | None:
    """Return the user behind a session token, or None if invalid/expired/revoked."""
    token_hash = session_helper.hash_token(raw_token)
    ws = await session.scalar(select(UserSession).where(UserSession.token_hash == token_hash))
    if ws is None or ws.revoked_at is not None or ws.expires_at < clock.now():
        return None
    return await session.get(User, ws.user_id)


async def revoke_session(session: AsyncSession, *, raw_token: str, clock: Clock) -> None:
