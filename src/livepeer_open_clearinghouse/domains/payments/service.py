...
async def _attempt_auto_replenish(
    ...
    *,
    ...
    grant_wei = increment_wei if room == Decimal("Infinity") else min(Decimal(increment_wei), room)
    if grant_wei <= 0:
        return
...
async def mint_payment(
    ...
    *,
    ...
    cfg = await billing_service.resolve_billing_config(session, user_id=user_id, settings=settings)
    ...
    if balance.amount_wei < funded_value_wei and cfg.auto_replenish_increment_wei > 0:
        await _attempt_auto_replenish(session, user_id=user_id, cfg=cfg, clock=clock)
        balance = await billing_service.get_balance(session, user_id=user_id)
