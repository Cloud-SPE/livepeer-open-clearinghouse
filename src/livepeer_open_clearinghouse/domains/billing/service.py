...
from livepeer_open_clearinghouse.domains.billing.repo import (
    CreditBalance,
    CreditLedger,
    CreditTopup,
    SpendWindow,
...
async def _ensure_balance_row(session: AsyncSession, *, user_id: uuid.UUID) -> CreditBalance:
...
def window_bounds_for(now: datetime, period_seconds: int) -> tuple[datetime, datetime]:
...
async def remaining_window_room(
    ...
    *,
    ...
    row = await session.scalar(
        select(SpendWindow).where(SpendWindow.user_id == user_id, SpendWindow.window_start == start)
    )
...
async def upsert_billing_config(
    ...
    *,
    ...
    row.auto_replenish_increment_wei = (
        Decimal(auto_replenish_increment_wei) if auto_replenish_increment_wei is not None else None
    )
    row.auto_replenish_threshold_wei = (
        Decimal(auto_replenish_threshold_wei) if auto_replenish_threshold_wei is not None else None
    )
...
async def run_auto_replenish(session: AsyncSession, *, clock: Clock, settings: Settings) -> int:
    ...
    user_ids: list[uuid.UUID] = list(await session.scalars(select(CreditBalance.user_id)))

    count = 0
    for user_id in user_ids:
        resolved = await resolve_billing_config(session, user_id=user_id, settings=settings)
