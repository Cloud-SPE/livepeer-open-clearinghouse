#!/usr/bin/env bash
# Restore a quiesced LOC 0024 database and prove the wholesale schema migration is inert.
...
fail() { printf '[fail] %s\n' "$*" >&2; exit 1; }
log() { printf '[wholesale-migration] %s\n' "$*" >&2; }
...
container_name="loc-wholesale-migration-rehearsal-$$"
...
log "taking a consistent read-only source dump"
PGSERVICE=loc_source PGSERVICEFILE="$source_service_file" pg_dump \
  --format=custom --no-owner --no-acl --serializable-deferrable \
  --file="$ARTIFACT_DIR/pre-wholesale.dump"
dump_end_ns="$(date +%s%N)"
sha256sum "$ARTIFACT_DIR/pre-wholesale.dump" > "$ARTIFACT_DIR/pre-wholesale.dump.sha256"
pg_restore --list "$ARTIFACT_DIR/pre-wholesale.dump" > "$ARTIFACT_DIR/pre-wholesale.dump.list"

log "starting isolated Postgres ${POSTGRES_IMAGE}"
docker run -d --rm \
  --name "$container_name" \
  -e POSTGRES_DB=loc_wholesale_migration \
  -e POSTGRES_USER=loc_wholesale_migration \
  -e "POSTGRES_PASSWORD=$staging_password" \
  -p 127.0.0.1::5432 \
  "$POSTGRES_IMAGE" >/dev/null
for _ in $(seq 1 60); do
  if docker exec "$container_name" pg_isready \
    -U loc_wholesale_migration -d loc_wholesale_migration >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
docker exec "$container_name" pg_isready \
  -U loc_wholesale_migration -d loc_wholesale_migration >/dev/null \
  || fail "isolated Postgres did not become ready"

staging_port="$(docker port "$container_name" 5432/tcp | sed 's/.*://')"
staging_url="postgresql+asyncpg://loc_wholesale_migration:${staging_password}@127.0.0.1:${staging_port}/loc_wholesale_migration"
restore_start_ns="$(date +%s%N)"
log "restoring source dump"
PGHOST=127.0.0.1 PGPORT="$staging_port" PGUSER=loc_wholesale_migration \
PGPASSWORD="$staging_password" PGDATABASE=loc_wholesale_migration pg_restore \
  --dbname=loc_wholesale_migration --exit-on-error --no-owner --no-acl \
  "$ARTIFACT_DIR/pre-wholesale.dump"
restore_end_ns="$(date +%s%N)"

log "auditing legacy drain and revision 0024"
DATABASE_URL="$staging_url" uv run python scripts/wholesale_migration_audit.py snapshot \
  --phase pre --output "$ARTIFACT_DIR/pre.json"
...
log "auditing legacy preservation and disabled wholesale initialization"
DATABASE_URL="$staging_url" uv run python scripts/wholesale_migration_audit.py snapshot \
  --phase post --output "$ARTIFACT_DIR/post.json"
uv run python scripts/wholesale_migration_audit.py compare \
...
uv run python - \
  "$ARTIFACT_DIR/timing.json" "$started_at" "$finished_at" \
...
path, started, finished, dump_ms, restore_ms, migration_ms = sys.argv[1:]
...
log "rehearsal passed; retain evidence from ${ARTIFACT_DIR}"
